...
class User {
    ...
    static async register(user) {
        try {
            let [dbUsers] = await pool.query("Select * from user where usr_name = ?", [user.name]);
            if (dbUsers.length)
                return {
                    status: 400, result: [{
                        location: "body", param: "name", msg: "That name already exists"
                    }]
                };
    ...
    static async saveToken(user) {
        try {
            let [result] =
                await pool.query(`Update user set usr_token=? where usr_id = ?`, [user.token,user.id]);
